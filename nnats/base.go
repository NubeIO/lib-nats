...
type IClient interface {
	...
	SubscribeSync(uuid string, subject string) (*nats.Subscription, error)
	...
	Unsubscribe(uuid string, sub *nats.Subscription) error
	...
	GetKVKeysByPrefix(uuid, bucket string, prefix string) ([]string, error)
...
// SubscribeSync listens for messages on a specific connection and returns a subscription.
func (n *Client) SubscribeSync(uuid string, subject string) (*nats.Subscription, error) {
	conn, err := n.getActiveConnection(uuid)
	if err != nil {
		return nil, err
	}
	...
	sub, err := conn.Connection.SubscribeSync(subject)
	if err != nil {
...
// Unsubscribe removes a subscription from a specific connection.
func (n *Client) Unsubscribe(uuid string, sub *nats.Subscription) error {
	if sub == nil {
	...
	if foundIndex != -1 {
		conn.Subjects = append(conn.Subjects[:foundIndex], conn.Subjects[foundIndex+1:]...)
	}
